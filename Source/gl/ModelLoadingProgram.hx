...
/**
 * The ModelLoadingProgram is used in the model loading and is called from the
 * Mesh objects. This is a change in that the program is passed to the object 
 * which then calls it with its vertex and other properties. Up to this point
 * the Scene has called programs directly after obtaining the vertex information
 * from the drawable objects.
 */
class ModelLoadingProgram extends Program
{
	// Shader source
	...
	public function new(gl:WebGLRenderContext, context:Context3D):Void
	{
		...
		Assets.getText("assets/shaders/modelLoading.frag");
	...
	public function getShaderVarLocations():Void
	{
		...
		for (i in 0...ModelLoadingScene.NUM_POINT_LIGHTS)
		{
