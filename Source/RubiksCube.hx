...
enum abstract Axis(Int) {
	...
	var Y = 1;
	var Z = 2;
	static final vectors = [0 => new Vector3D(1, 0, 0),
...
class RubiksCube {
	...
	static final ORANGE:RGBA = 0xFF8000ff;
    ...
    public function new(context:Context3D, x:Int, y:Int, z:Int, scene:Scene) {
        final SIDE = 64;
        final START_OFFSET = -(ROW_LEN * SIDE) / 2 + SIDE / 2;
        ...
        for (i in 0...ROW_LEN) {   // X front face - left -> right
            for (j in 0...ROW_LEN) {   // Y front face - bottom -> top
                for (k in 0...ROW_LEN) {  // Z front face 0 - front -> back
                    // skip interior cubes
                    if (i > 0 && i < ROW_LEN - 1 &&
                        j > 0 && j < ROW_LEN - 1 &&
                        k > 0 && k < ROW_LEN - 1) {
                    ...
                    scaleMatrix.appendScale(SIDE, SIDE, SIDE);
                    var rotationMatrix = new Matrix3D();
                    rotationMatrix.identity();
                    var translationMatrix = new Matrix3D();
                    translationMatrix.appendTranslation(START_OFFSET + i * SIDE, START_OFFSET + j * SIDE, START_OFFSET + k * SIDE);
                    ...
                    var cd:CubeData = { id: '$i' + '$j' + '$k' , x:i, y:j, z:k, cube:c,
                                        scaleMatrix: scaleMatrix, rotationMatrix: rotationMatrix,
                                        translationMatrix: translationMatrix, modelMatrix: modelMatrix};
                    _cubes[cd.id] = cd;
                }
            }
        }
    ...
    function updateLocations(angle:Float, axis:Axis):Void {
        ...
        switch(axis) {
            case X:
                ...
                tmpCubes.sort((a, b) -> {return comp(a.y, b.y);});
                ...
                tmpCubes.sort((a, b) -> {return comp(a.z, b.z);});
                sortedCubes = sortedCubes.concat(tmpCubes);
                for (r in cubesToRm) {
                    cubesToUpdate.remove(r);
                }
                ...
                tmpCubes.sort((a, b) -> {return comp(b.y, a.y);});
                sortedCubes = sortedCubes.concat(tmpCubes);
                for (r in cubesToRm) {
                    cubesToUpdate.remove(r);
                }
                ...
                tmpCubes.sort((a, b) -> {return comp(b.z, a.z);});
                sortedCubes = sortedCubes.concat(tmpCubes);
                for (r in cubesToRm) {
                    cubesToUpdate.remove(r);
                }
            ...
            case Y:
                ...
                tmpCubes.sort((a, b) -> {return comp(a.x, b.x);});
                sortedCubes = sortedCubes.concat(tmpCubes);
                for (r in cubesToRm) {
                    cubesToUpdate.remove(r);
                }
                ...
                tmpCubes.sort((a, b) -> {return comp(b.x, a.x);});
                sortedCubes = sortedCubes.concat(tmpCubes);
                for (r in cubesToRm) {
                    cubesToUpdate.remove(r);
                }
        ...
        if (angle > 0) {
            ...
            for (i in 0...ROW_LEN - 1) {
                tmpCubePos.push({id: sortedCubes[i].id,
                    x: sortedCubes[i].x,
                    y: sortedCubes[i].y,
                    z: sortedCubes[i].z
                });
